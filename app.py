...
def filter_profanity(text):
    profane_words = {
        "dog", "bastard", "dick", "pussy", "scoundrel", "fuck", "fucking", "shit", "bitch", "cunt", "asshole"
    }
    words = text.split()
    filtered_words = []
    for word in words:
...
def get_trending_crypto():
    try:
        r = requests.get("https://api.coingecko.com/api/v3/search/trending", timeout=6, headers={"Accept": "application/json"})
        r.raise_for_status()
        data = r.json()
...
def get_crypto_price(topic):
    try:
        r = requests.get(f"https://api.coingecko.com/api/v3/simple/price?ids={topic.lower()}&vs_currencies=usd", timeout=6, headers={"Accept": "application/json"})
        r.raise_for_status()
        data = r.json()
